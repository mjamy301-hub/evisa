...
export async function GET(req: Request) {
  ...
  const where = search ? { Username: { contains: search, mode: "insensitive" as const } } : undefined;
...
export async function POST(req: Request) {
  ...
  const user = await prisma.user.create({ data: { Username: parsed.data.username, PasswordHash: hash, Role: "USER" } });
  await prisma.application.create({ data: { UserId: user.Id, Status: "NEW" } });
