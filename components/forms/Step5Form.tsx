...
type UploadedMap = Partial<Record<FileIdentifier, { Url: string; FileName: string }>>;
  ...
  useEffect(() => {
    (async () => {
      ...
      const mapped = json.data.reduce(
        (acc: UploadedMap, curr: { Identifier: FileIdentifier; Url: string; FileName: string }) => {
          if (!acc[curr.Identifier]) {
            return { ...acc, [curr.Identifier]: { Url: curr.Url, FileName: curr.FileName } };
          }
          return acc;
        },
        {}
  ...
  return (
    <div>
      ...
      <Link
        ...
        className={clsx("text-blue-500 hover:underline font-medium mb-6 block", {
